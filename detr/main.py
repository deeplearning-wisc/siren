...
from engine import evaluate, evaluate_ood_id, evaluate_ood_ood, train_one_epoch, viz, evaluate_ood_id_speckle
...
def main(args):
    ...
    if args.eval:
        if 'ood' in args.dataset:
            ...
            test_stats, coco_evaluator = evaluate_ood_id(args, model, criterion, postprocessors,
                                                  ...
                                                  args.resume.split('checkpoint.pth')[0], args.dataset, args.viz_prediction_results)
        if args.output_dir and 'ood' not in args.dataset:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, swav_model, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm, args)
        lr_scheduler.step()
        if args.output_dir:
        ...
        if args.dataset in ['coco', 'voc', 'bdd'] and epoch % args.eval_every == 0:
            test_stats, coco_evaluator = evaluate(
                model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args.dataset, args
            )
        else:
