...
labels_val_in = torch.from_numpy(np.load('/nobackup-slow/dataset/my_xfdu/detr_out/exps/bdd/id-logits.npy'))
labels_val_in = torch.sigmoid(labels_val_in).max(-1)[1]

labels_val_in = labels_val_in[:100000]
labels_ood = torch.from_numpy(np.load('/nobackup-slow/dataset/my_xfdu/detr_out/exps/bdd/ood-logits.npy'))
