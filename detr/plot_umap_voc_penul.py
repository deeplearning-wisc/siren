...
if args.logits:
    data = np.load('/nobackup/my_xfdu/detr_out/exps/' + args.name + '/id-logits_maha_train.npy')#.item()
    data_pen = np.load('/nobackup/my_xfdu/detr_out/exps/' + args.name + '/id-pen_maha_train.npy')#.item()
    shape = 20
elif args.pro:
    data = np.load('/nobackup/dataset/my_xfdu/detr_out/exps/' + args.name + '/id-pro_maha_train.npy')  # .item()
    shape = args.pro_length
else:
    data = np.load('/nobackup/my_xfdu/detr_out/exps/' + args.name + '/id-pen_maha_train.npy')  # .item()
    shape = 256
...
fixed_number = 460
for i in range(20):
    indices = np.random.choice(len(data[i]), fixed_number, replace=False)
    if i == 0:
        data_preprocess = data[i][indices]
    else:
        data_preprocess = np.concatenate((data_preprocess, data[i][indices]), 0)
...
for i in range(0, 20):
    plt.scatter(embedding[:, 0][sum: sum + number_class[i]],
                ...
                c=color(i),
                label=new_class_dict[index], cmap="Spectral", s=1)

    plt.text(embedding[:, 0][sum: sum + number_class[i]][-1], embedding[:, 1][sum: sum + number_class[i]][-1], new_class_dict[i]+ '_' + str(kappa[i]))
    sum += number_class[i]#len(data_dict[i])
    index += 1
plt.legend(fontsize=20)
ax.legend(loc='lower left',markerscale=9)
plt.setp(ax, xticks=[], yticks=[])
plt.savefig('./voc_coco_umap_visual_center-pro-dim-16.jpg', dpi=250)
# plt.show()