...
# # breakpoint()
if args.normalize:
    id_val_norm = np.linalg.norm(all_data_in, ord=2, axis=-1)
    ood_val_norm = np.linalg.norm(all_data_out, ord=2, axis=-1)
...
index = faiss.IndexFlatIP(id_train_data.shape[1])
index.add(id_train_data)
for K in [1]:
# for K in [10]:
    D, _ = index.search(all_data_in, K)
    scores_in = D[:,-1] * id_val_norm
    all_results = []
    all_score_ood = []
    # for ood_dataset, food in food_all.items():
    D, _ = index.search(all_data_out, K)
    scores_ood_test = D[:,-1] * ood_val_norm
