...
if args.gpu == 0:
    prefix = '/nobackup-slow/'
else:
    prefix = '/nobackup/'

# ID data
id_data = pickle.load(open(prefix + '/dataset/my_xfdu/VOC-Detection/' + args.model + '/'+args.name+'/random_seed'+'_' +str(args.seed)  +'/inference/voc_custom_val/standard_nms/corruption_level_0/probabilistic_scoring_res_odd_'+str(args.thres)+'.pkl', 'rb'))
if args.open:
    ood_data = pickle.load(open(
        prefix + 'dataset/my_xfdu/VOC-Detection/' + args.model + '/' + args.name + '/random_seed' + '_' + str(
            args.seed) + '/inference/openimages_ood_val/standard_nms/corruption_level_0/probabilistic_scoring_res_odd_' + str(
            args.thres) + '.pkl', 'rb'))
else:
    ood_data = pickle.load(open(prefix + 'dataset/my_xfdu/VOC-Detection/' + args.model + '/'+args.name+'/random_seed' +'_'+str(args.seed)  +'/inference/coco_ood_val/standard_nms/corruption_level_0/probabilistic_scoring_res_odd_'+str(args.thres)+'.pkl', 'rb'))
...
# assert len(id_data['inter_feat'][0]) == 23# + 1024
...
if args.energy:
    ...
    id_score = -np.max(F.softmax(torch.stack(id_data['inter_feat']) / args.T, dim=1)[:,:-1].cpu().data.numpy(), axis=1)
    ood_score = -np.max(F.softmax(torch.stack(ood_data['inter_feat']) / args.T, dim=1)[:,:-1].cpu().data.numpy(), axis=1)
# id_score = -np.max(F.softmax(id_data, dim=1).cpu().data.numpy(), axis=1)
# ood_score = -np.max(F.softmax(ood_data, dim=1).cpu().data.numpy(), axis=1)
